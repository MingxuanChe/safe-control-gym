...
class PendulumStateConstraint(BoundedConstraint):
    """Constrain the pendulum's state to the observation space bounds.
    """
    def __init__(self, 
                 env, 
                 low=None, 
    ...
    def is_violated(self, env):
        ...
        Returns:
            bool: Whether the constraint is violated.
...
class PendulumInputConstraint:
    """Constrain the pendulum's input to the action space bounds.
    ...
    def __init__(self,
        ...
        """
        if high is None:
            self.high = np.array([env.max_torque])
        else:
            assert len(high) == env.action_space.shape[0]
            self.high = high
        if low is None:
            self.low = -1 * np.array([env.max_torque])
        else:
...
class PendulumSymmetricStateConstraint(BoundedConstraint):
    ...
    def __init__(self,
        ...
        self.bound =  np.array(bound, ndim=1)
        self.super = super().__init__(env, 
                                      lower_bounds= -bound,
                                      upper_bounds=bound,
...
class PendulumBoundConstraint(Constraint):
