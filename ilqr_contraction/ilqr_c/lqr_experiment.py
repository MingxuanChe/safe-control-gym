...
def run(gui=True, n_episodes=1, n_steps=None, save_data=True):
    ...
    assert os.path.exists(f'./config_overrides/{SYS}/{SYS}_{TASK}.yaml'), f'./{ALGO}/config_overrides/{SYS}/{SYS}_{TASK}.yaml does not exist'
    assert os.path.exists(f'./config_overrides/{SYS}/{ALGO}_{SYS}_{TASK}_{PRIOR}.yaml'), f'./{ALGO}/config_overrides/{SYS}/{ALGO}_{SYS}_{TASK}_{PRIOR}.yaml does not exist'
...
def post_analysis(state_stack, input_stack, env):
    ...
    z = state_stack[:, 2]
    x = state_stack[:, 0]
    axs.plot(x, z, 'b', label='Agent Trajectory')
    ref = env.X_GOAL
    axs.plot(ref[:, 0], ref[:, 2], 'r--', label='Reference')
    axs.scatter(x[0], z[0], color='g', marker='o', s=100, label='Initial State')
    axs.set_xlabel('X')
    axs.set_ylabel('Z')
    axs.legend()
    plt.show()
