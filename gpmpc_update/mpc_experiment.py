...
def run(gui=True, n_episodes=1, n_steps=None, save_data=False):
    ...
    assert os.path.exists(f'{cur_script_path}/{ALGO}/config_overrides/{SYS}/{SYS}_{TASK}.yaml'), f'{cur_script_path}/{ALGO}/config_overrides/{SYS}/{SYS}_{TASK}.yaml does not exist'
    assert os.path.exists(f'{cur_script_path}/{ALGO}/config_overrides/{SYS}/{ALGO}_{SYS}_{TASK}_{PRIOR}.yaml'), f'{cur_script_path}/{ALGO}/config_overrides/{SYS}/{ALGO}_{SYS}_{TASK}_{PRIOR}.yaml does not exist'
    if SAFETY_FILTER is None:
        sys.argv[1:] = ['--algo', ALGO,
                        '--task', agent,
                        '--overrides',
                            f'{cur_script_path}/{ALGO}/config_overrides/{SYS}/{SYS}_{TASK}.yaml',
                            f'{cur_script_path}/{ALGO}/config_overrides/{SYS}/{ALGO}_{SYS}_{TASK}_{PRIOR}.yaml',
                        '--seed', '2',
                        '--use_gpu', 'True',
    ...
    for _ in range(n_episodes):
        ...
        if SAFETY_FILTER is None:
            ...
            if ALGO in ['gpmpc_acados', 'gp_mpc'] and config.algo_config.gp_model_path is not None:
        ...
        fig_save_path = f'./{config.algo_config.output_dir}'
        post_analysis(trajs_data['obs'][0], trajs_data['action'][0], ctrl.env, fig_save_path)
...
def post_analysis(state_stack, input_stack, env, save_path=None):
